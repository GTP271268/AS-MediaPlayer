package com.gtp.androidplayer.custom;
...
public class MysurfaceView extends SurfaceView {
    ...
    public interface OnVideoPlayingListener {
        ...
        void onAwake(MotionEvent event);

        void onSlideChangeProgress(float persent);

        void onLightChange(int persent);

        void onSoundChange(int persent);
    }
    ...
    public void initChangeSize(float width, float height) {
           ...
           // mediaPlayer.setDataSource(url);
            ...
            mediaPlayer.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
                ...
                public void onPrepared(final MediaPlayer mediaPlayer) {
                    ...
                    surWidth = getWidth();
                    surHeight = getHeight();
                    initChangeSize(systemWidth, systemHeight);
                    //scaleChangeSize(surWidth, surHeight);
                    mediaPlayer.start();
    ...
    class MyOnGestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            Log.i("yy", "wh" + windowWidth + "/" + windowHeight);
            if (isFullScreen) {
                if (Math.abs(mOldY - y) > 50) {
                    if (mOldX > windowWidth * 0.5) {// 右边上下滑动
                        onVolumeSlide(((mOldY - y) / windowHeight / 2));
                    } else {// 左边上下滑动

                        onBrightnessSlide((mOldY - y) / windowHeight / 2);
                    }
                }
                listener.onSlideChangeProgress(X - mOldX);
            } else {
                if (Math.abs(mOldY - y) > 200) {
                    if (mOldX > windowWidth * 0.4) {// 右边上下滑动
                        Log.i("yy", "右上下" + (mOldY - y));
                        onVolumeSlide(((mOldY - y + 300) / windowHeight / 2));
                    } else {// 左边上下滑动

                        onBrightnessSlide((mOldY - y + 240) / windowHeight / 2);
                        Log.i("yy", "左上下" + (mOldY - y));
                    }
                }
                listener.onSlideChangeProgress(X - mOldX - 350);
            }
            //Log.i("yy", "上下"+(mOldY - y));
            Log.i("yy", "左右" + (X - mOldX));
    ...
    private void onVolumeSlide(float percent) {
        ...
        Log.i("gtp", "音量 " + index);
        // 变更声音
        listener.onSoundChange((index * 20) / 3);
    ...
    private void onBrightnessSlide(float percent) {
        ...
        Log.i("gtp", "亮度 " + lpa.screenBrightness);
        listener.onLightChange((int) (lpa.screenBrightness * 100));
...
//        else{
//            Toast.makeText(context, "只有6.0及以上设备才支持倍数播放", Toast.LENGTH_SHORT).show();
//        }
